...
class ChunkManager {
  public:
    ...
    ChunkManager(uint32_t threadCount = std::thread::hardware_concurrency() / 2);
    ~ChunkManager();
  ...
  private:
    ...
    std::priority_queue<ChunkGenRequest, std::vector<ChunkGenRequest>, RequestCompare> m_RequestQueue;
    std::queue<Chunk*> m_DirtyQueue;
