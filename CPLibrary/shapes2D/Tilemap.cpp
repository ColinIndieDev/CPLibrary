...
namespace CPL {
    ...
    void Tilemap::BeginEditing() {
        batches.clear();
    }

    void Tilemap::AddTile(const glm::vec2 position, const glm::vec2 size, const Texture2D* texture) {
        if (!texture || texture->texture == 0) return;
        float quad[] = {
    ...
    void Tilemap::Draw(const Shader& shader) {
        ...
        shader.SetVector3f("offset", glm::vec3(glm::vec2(0, 0), 0.0f));
        ...
        for (auto& [id, batch] : batches) {
            ...
            glDrawArrays(GL_TRIANGLES, 0, static_cast<GLsizei>(batch.vertices.size() / 5));
